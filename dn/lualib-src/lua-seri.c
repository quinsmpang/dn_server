...
static char *
_seri_args(lua_State *L, struct block *b, size_t * sz) {
	uint32_t len = 0;
	memcpy(&len, b->buffer ,sizeof(len));
...
char *
_pack_args(lua_State *L, size_t index, size_t *len) {
	if (lua_gettop(L) < index) {
		return NULL;
	}
	struct write_block wb;
	wb_init(&wb, NULL);
	_pack_from(L, &wb, index-1);
	struct block *b = wb_close(&wb);
	char *args = _seri_args(L, b, len);
	while(b) {
		struct block * next = b->next;
		free(b);
		b = next;
...
int
_unpack_args(lua_State *L, char *buffer, size_t len) {
	int n = lua_gettop(L);
	struct read_block rb;
	rball_init(&rb, buffer, len);
