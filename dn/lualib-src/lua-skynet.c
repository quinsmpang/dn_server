...
_command(lua_State *L) {
	...
	int n = lua_gettop(L);
	if (n == 2) {
		parm = strdup(luaL_checkstring(L,2));
	} else if (n == 3) {
		assert(!strcmp(cmd, "LAUNCH"));
		const char * service = luaL_checkstring(L, 2);
		// 'LAUNCH snlua [service]' with no args
		const char * args = luaL_checkstring(L, 3);
		int len = strlen(service) + strlen(args) + 2;
		parm = malloc(len);
		snprintf(parm, len, "%s %s", service, args);
	} else if (n > 3) {
		size_t sz;
		char * buffer = _pack_args(L, 4, &sz);
		const char * service = luaL_checkstring(L, 2);
		const char * args = luaL_checkstring(L, 3);
		int len = strlen(service) + strlen(args) + 2 + sizeof(int) + sz;
		parm = malloc(len);
		int ptr = snprintf(parm, len, "%s %s ", service, args);
		memcpy(parm+ptr, &sz, 4);
		memcpy(parm+ptr+4, buffer, sz);
		free(buffer);
	}
...
_send(lua_State *L) {
	...
	default:
		luaL_error(L, "skynet.send invalid param %s", lua_typename(L,4));
	}
	if (session < 0) {
		luaL_error(L, "skynet.send session (%d) < 0", session);
	}
