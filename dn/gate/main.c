...
_cb(struct skynet_context * ctx, void * ud, int type, int session, uint32_t source, const void * msg, size_t sz) {
	...
	if (type == PTYPE_TEXT) {
		...
		if (agent) {
			...
			if (g->header == 'W') {
				...
				int head_len;
				uint8_t *head;
				head_len = data_len>125 ? 4 : 2;
				head = (uint8_t *)(msg + (6 - head_len));
				head[0] = 0x82;
				head[1] = data_len>125 ? 126 : data_len;
				if (head_len == 4) {
					head[2] = data_len >> 8 & 0xff;
					head[3] = data_len & 0xff;
				}
				mread_push(m, id, (void *)head, data_len + head_len, (void *)data);
				return 1;
			}
#endif
	...
	if (connection_id < 0) {
		...
		if (id == 0) {
			...
			_report(g, ctx, "%d open %d %s:%u",id,fd, mread_client_ip(m, connection_id), mread_client_port(m, connection_id));
		}
#ifdef WEBSOCKET
		if (g->header == 'W') {
			assert(g->header_size == 2);
			struct ws_frame_header *header = (struct ws_frame_header *)plen;
			// todo: support header->fin == 0
			if (header->fin == 0) {
				skynet_error(ctx, "Websocket error, header->fin == 0");
				mread_close_client(g->pool, connection_id);
				goto _break;
			}
			printf("Websocket payload len: %d, opcode: %d\n", header->payload_len, header->opcode);
			int data_len;
			uint8_t *mask;
			if (header->payload_len == 126) {
				...
				data_len = plen[0] << 24 | plen[1] << 16 | plen[2] << 8 | plen[3];
			}
			else {
			...
			if (header->mask) {
				...
				skynet_error(ctx, "Client websocket message has no mask.\n");
			}
			uint8_t *data = mread_pull(m, data_len);
			if (data == NULL) {
				if (mread_closed(m)) {
					_remove_id(g, id);
					_report(g, ctx, "%d close", id);
			...
			_forward(ctx, g, id, data, data_len);
			mread_yield(m);
			goto _break;
		}
#endif
		...
		if (g->header == 'P') {
			int fd = mread_socket(m , connection_id);
			if (plen[0] == '<' && plen[1] == 'p') {
				static const char * policy_req = "<policy-file-request/>";
				static const char * policy_resp = "<cross-domain-policy><allow-access-from domain=\"*\" to-ports=\"*\" /></cross-domain-policy>\0";
				static int resp_len = 89;
				if ( 0 == strncmp(policy_req, (char *)plen, 22)) {
					send(fd, policy_resp, resp_len, 0);
				}
			}
			mread_close_client(g->pool,connection_id);
		}
...
gate_init(struct gate *g , struct skynet_context * ctx, char * parm) {
	...
	if (header != 'S' && header !='L' && header != 'W' && header != 'P') {
		skynet_error(ctx, "Invalid data header style '%c'", header);
		return 1;
	}
	g->header = header;
	if (client_tag == 0) {
	...
	if (g->header == 'W') {
#ifndef WEBSOCKET
		skynet_error(ctx, "Not support websocket currently, add -DWEBSOCKET to makefile then execute 'make'\n");
#endif
		g->header_size = 2;
	}
	else if (g->header == 'P') {
		// length of flash 843 request
		g->header_size = 23;
	}
	else {
		g->header_size = g->header=='S' ? 2 : 4;
	}
