...
_init(struct snlua *l, struct skynet_context *ctx, const char * args) {
	...
	if (r != 0) {
		if (r<0) {
			skynet_error(ctx, "lua parser [%s] load error : file not found", filename);
		} else {
...
_launch(struct skynet_context * context, void *ud, int type, int session, uint32_t source , const void * msg, size_t sz) {
	...
	if (_init(l, context, msg)) {
		// "0" for init failed
		skynet_sendname(context, ".launcher", PTYPE_TEXT, 0, "0", 1);
...
snlua_init(struct snlua *l, struct skynet_context *ctx, const char * args) {
	int sz;
	const char * parm = strchr(args, ' ');
	if (parm == NULL) {
		sz = strlen(args)+1;
	} else {
		parm += 1;
		sz = parm - args + *(int *)parm + 4;
	}
