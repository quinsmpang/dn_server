...
function suspend(co, result, command, param, size)
	...
	elseif command == "RETURN" then
		...
		-- c.send maybe throw an error, so call trace_count first.
...
function skynet.launch(...)
	local name = ...
	name = string.match(name, "(%g+)")
	assert(name ~= "snlua", "Using skynet.newservice to launch lua service.")
...
function skynet.rawret(addr, typename, session, ...)
	local p = proto[typename]
	return c.send(addr, 1, session, p.pack(...))
end
...
function skynet.newservice(name, ...)
	local handle = skynet.call(".launcher", "lua", "LAUNCH", name, ...)
	if handle == nil then
		return nil
	else
...
function skynet.start(start_func)
	...
	skynet.timeout(0, function()
		init_template(start_func or function() end)
...
function skynet.trace()
	return assert(c.trace_new(trace_handle))
end
...
function skynet.suspend(co, ...)
	return suspend(co, ...)
end
skynet.error = c.error

if skynet.getenv"runasdaemon" == "1" then
	print = function() end
end
